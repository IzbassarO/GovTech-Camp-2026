"""Strict Curated Dataset v1 input validation for P4.

The ACCEPTED dataset contract already exists as the curated Pydantic models
(``extra="forbid"``, full nested typing, ID/hash patterns); P4 validates every
input record against those models BEFORE extraction, plus an explicit
supported-schema-version gate. Expected data problems become a concise
``P4RunError`` carrying the file, the JSONL line, the field path and a
...
def validate_input_records(
    ...
    contract. Raises ``error_type`` (P4RunError) on the first violation."""
